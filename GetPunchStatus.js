const $ = new Env('punchStatus');

main();

async function main() {
    const nowDateTime = $.time('yyyy-MM-dd HH:mm:ss');
    const today = nowDateTime.split(' ')[0];
    const nowTime = nowDateTime.split(' ')[1];

    let punchType;

    if (nowTime < '17:30:00') {
        punchType = '上班卡';
    } else if (nowTime >= '17:30:00' && nowTime < '20:35:00') {
        punchType = '下班卡';
    } else {
        punchType = '加班卡';
    }

    const todayIsWorkDay = await isWorkDay(today);
    const returnPunchStatus = (status) => {
        $.done(JSON.stringify({ punchType: punchType, punchStatus: status, isWorkDay: todayIsWorkDay }));
    };

    if (!todayIsWorkDay) {
        $.log('今天是休息日, punchStatus: true');
        returnPunchStatus(true);
        return;
    }

    const skipPunchExpire = $.getjson('skip_punch_expire', null);
    if (skipPunchExpire && nowDateTime <= skipPunchExpire) {
        $.log(`跳过了本次打卡, 失效时间: ${skipPunchExpire}, 本次 punchStatus: true`);
        returnPunchStatus(true);
        return;
    }

    const punchInfo = $.getjson('punch_info', {});
    try {
        const hasTodayRecord = punchInfo.date === today && punchInfo.first_punch;
        if (!hasTodayRecord) returnPunchStatus(false);

        const { first_punch, last_punch } = punchInfo;
        $.log(`${today} 打卡记录: ${first_punch} ${last_punch}`);
        const statusMap = {
            ['上班卡']: first_punch <= '09:05:59',
            ['下班卡']: last_punch >= '17:30:00',
            ['加班卡']: last_punch >= '20:35:00',
        };

        returnPunchStatus(statusMap[punchType] || false);
    } catch (error) {
        $.error(`获取打卡状态时出错: ${error.message}`);
        returnPunchStatus(false);
    }
}
...
async function isWorkDay(date) {
    ...
    try {
        ...
        // 先尝试从缓存获取节假日数据
        let holiday_cn = $.getjson(`holiday_cn_${year}`, null);
        if (!holiday_cn) {
            // 缓存中没有数据，从API获取
            $.log(`获取${year}年节假日数据...`);
            const holiday_cn_url = `https://raw.githubusercontent.com/NateScarlet/holiday-cn/master/${year}.json`;
            holiday_cn = await $.http.get(holiday_cn_url).then((res) => JSON.parse(res.body).days);
            // 将数据存储到缓存
            $.setjson(holiday_cn, `holiday_cn_${year}`);
            $.log(`${year}年节假日数据已缓存`);
        }

        const holidayInfo = holiday_cn.find((item) => item.date === date);

        if (holidayInfo) {
            // 节假日逻辑
            $.log(`${date} ${holidayInfo.isOffDay ? '是法定节假日, 休息' : '是法定工作日, 上班'}`);
            return !holidayInfo.isOffDay;
        }

        // 月末周六特殊逻辑
        const currentDate = new Date(date);
        const afterCurrentDate = (daysNum) => {
            const newDate = new Date(currentDate);
            newDate.setDate(currentDate.getDate() + daysNum);
            return newDate;
        };
        if (dayOfWeek === 6) {
            // 如果下一个周六的月份不同，说明当前是月末最后一个周六
            if (currentDate.getMonth() !== afterCurrentDate(7).getMonth()) {
                // 获取下一天（周日）
                const nextDay = afterCurrentDate(1);
                const nextDayStr = nextDay.toISOString().split('T')[0];
                const nextDayHolidayInfo = holiday_cn.find((item) => item.date === nextDayStr);

                if (nextDayHolidayInfo && !nextDayHolidayInfo.isOffDay) {
                    $.log(`${date} 月末周六，次日是法定工作日，休息`);
                    return false;
                } else {
                    $.log(`${date} 月末周六，上班`);
                    return true;
        ...
        $.error(`获取节假日数据时出错: ${error.message}`);
    }

    // 常规逻辑：周末休息，工作日上班
    $.log(`${date} ${isWeekend ? '是周末，休息' : '是工作日, 上班'}`);
    return !isWeekend;
}
