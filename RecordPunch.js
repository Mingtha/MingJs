...
if (typeof $response === 'undefined') {
    ...
    const second = Math.floor(Math.random() * 60)
        .toString()
        .padStart(2, '0');
    $.log(`测试数据: ${date} ${hour}:${minute}:${second}`);
    $response = {
        body: `{"msg": "打卡成功", "data": {"sysDate": "${date} ${hour}:${minute}:${second}"}}`,
    };
}
...
function main() {
    ...
    if (bodyObj.msg === '打卡成功') {
        const punchDateTime = bodyObj.data?.sysDate ?? bodyObj.data?.time;
        if (!punchDateTime) {
            $.error(`未获取到打卡时间, 请检查 body: ${JSON.stringify($response.body)}`);
            return;
        }
        $.log(`打卡时间: ${punchDateTime}`);
        const punchDate = punchDateTime.split(' ')[0];
        const punchTime = punchDateTime.split(' ')[1];
        let punchInfo = $.getjson('punch_info', {});

        if (punchInfo.date !== punchDate) {
            // 新的一天
            punchInfo = {};
            punchInfo.date = punchDate;
            punchInfo.first_punch = punchTime;
            punchInfo.last_punch = punchTime;
        } else {
            if (punchTime < punchInfo.first_punch) {
                punchInfo.first_punch = punchTime;
            }
            if (punchTime > punchInfo.last_punch) {
                punchInfo.last_punch = punchTime;
            }
        }

        $.setjson(punchInfo, 'punch_info');
        $.info(`记录打卡成功 - ${punchInfo.date}: ${punchInfo.first_punch} - ${punchInfo.last_punch}`);
    } else {
        $.info(`记录打卡失败 - ${bodyObj.msg}`);
    }
}
